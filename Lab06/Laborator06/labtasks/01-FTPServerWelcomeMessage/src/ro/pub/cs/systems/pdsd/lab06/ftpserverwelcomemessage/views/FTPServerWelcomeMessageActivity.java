		...
		public void run() {
			try {
				...
				Socket socket = new Socket(
						FTPServerAddressEditText.getText().toString(), Constants.FTP_PORT);
				BufferedReader bufferReader = Utilities.getReader(socket);
				content = bufferReader.readLine();
				if(content.startsWith(Constants.FTP_MULTILINE_START_CODE)){
					content = bufferReader.readLine();
					if(content != null)
					while(!content.equals(Constants.FTP_MULTILINE_END_CODE1) ||
							!content.startsWith(Constants.FTP_MULTILINE_END_CODE2)){
						welcomeMessageTextView.post(new Runnable() {
							final String content_copy = content;
							@Override
							public void run() {
								// TODO Auto-generated method stub
								welcomeMessageTextView.append(content_copy);
							}
						});
						content = bufferReader.readLine();
